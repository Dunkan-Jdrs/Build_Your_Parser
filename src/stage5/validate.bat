...
ctest --test-dir ../../build -R "^Stage5_" --output-on-failure > "%TEMP%\byp_output.txt" 2>&1
set RESULT=%errorlevel%

type "%TEMP%\byp_output.txt"

if %RESULT% == 0 (
	echo %GREEN%Congratulation! You have a base of a CLI Parser. Add it to your portfolio, enhance it, and create new application using it.%RESET%
) else (
	echo.
	echo %YELLOW%==============================%RESET%
